...
function dateFormatsArray()
{
    ...
    foreach (dateFormats() as $formatKey => $formatValue) {
        $arr[] = Date::now()->format($formatValue);
    }
    return $arr;
}
...
{
    Date::setLocale(getLang());
    $format = Date::parse($date)->format(dateFormats()[settings('date_format')]);
    return $format;
}
...
function deleteUserNotification($id, $link)
{
    $notifications = UserNotification::where([['user_id', $id], ['link', $link]])->get();
    if ($notifications->count() > 0) {
        foreach ($notifications as $notification) {
            $notification->delete();
        }
...
{
    return UploadSettingsManager::handler();
}

function allowedTypes()
{
    return "image/jpg,image/jpeg,image/png,image/bmp,image/gif,image/webp,video/mp4,video/mpeg,video/mov,video/quicktime";
}

function autoDeletePeriods()
{
    $arr = [
        0 => ["title" => lang("Don't autodelete", "upload zone"), "days" => 0, "datetime" => null],
        1 => ["title" => lang("After 5 minutes", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addMinutes(5)],
        2 => ["title" => lang("After 15 minutes", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addMinutes(15)],
        3 => ["title" => lang("After 30 minutes", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addMinutes(30)],
        4 => ["title" => lang("After 1 hour", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addHour()],
        5 => ["title" => lang("After 3 hours", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addHours(3)],
        6 => ["title" => lang("After 6 hours", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addHours(6)],
        7 => ["title" => lang("After 12 hours", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addHours(12)],
        8 => ["title" => lang("After 1 day", "upload zone"), "days" => 1, "datetime" => Carbon::now()->addDay()],
        9 => ["title" => lang("After 2 days", "upload zone"), "days" => 2, "datetime" => Carbon::now()->addDays(2)],
        10 => ["title" => lang("After 3 days", "upload zone"), "days" => 3, "datetime" => Carbon::now()->addDays(3)],
        11 => ["title" => lang("After 4 days", "upload zone"), "days" => 4, "datetime" => Carbon::now()->addDays(4)],
        12 => ["title" => lang("After 5 days", "upload zone"), "days" => 5, "datetime" => Carbon::now()->addDays(5)],
        13 => ["title" => lang("After 6 days", "upload zone"), "days" => 16, "datetime" => Carbon::now()->addDays(6)],
        14 => ["title" => lang("After 1 week", "upload zone"), "days" => 7, "datetime" => Carbon::now()->addWeek()],
        15 => ["title" => lang("After 2 weeks", "upload zone"), "days" => 14, "datetime" => Carbon::now()->addWeeks(2)],
        16 => ["title" => lang("After 3 weeks", "upload zone"), "days" => 21, "datetime" => Carbon::now()->addWeeks(3)],
        17 => ["title" => lang("After 1 month", "upload zone"), "days" => 30, "datetime" => Carbon::now()->addMonth()],
        18 => ["title" => lang("After 2 months", "upload zone"), "days" => 60, "datetime" => Carbon::now()->addMonths(2)],
        19 => ["title" => lang("After 3 months", "upload zone"), "days" => 90, "datetime" => Carbon::now()->addMonths(3)],
        20 => ["title" => lang("After 4 months", "upload zone"), "days" => 120, "datetime" => Carbon::now()->addMonths(4)],
        21 => ["title" => lang("After 5 months", "upload zone"), "days" => 150, "datetime" => Carbon::now()->addMonths(5)],
        22 => ["title" => lang("After 6 months", "upload zone"), "days" => 180, "datetime" => Carbon::now()->addMonths(6)],
        23 => ["title" => lang("After 1 year", "upload zone"), "days" => 365, "datetime" => Carbon::now()->addYear()],
    ];
    if (!is_null(uploadSettings()->upload->files_duration)) {
        $periods = [];
        foreach ($arr as $key => $value) {
            if ($value['days'] > 0 && $value['days'] <= uploadSettings()->upload->files_duration) {
                $periods[] = $value;
            }
        }
        return $periods;
...
function filePasswordSession($shared_id)
{
    $prefix = "password_";
    return $prefix . $shared_id;
}

function fileDownloadSession($shared_id)
{
    $prefix = "download_";
    return $prefix . $shared_id;
}
...
{
    $reasons = [
        0 => lang('Privacy, copyright or legal complaints', 'image page'),
        1 => lang('Spam or misleading', 'image page'),
        2 => lang('Malware, virus or malicious content', 'image page'),
        3 => lang('Child abuse', 'image page'),
        4 => lang('Other', 'image page'),
    ];
    return $reasons;
}
...
function fileUser($fileEntry)
{
    if ($fileEntry->user_id) {
        $data['name'] = $fileEntry->user->firstname . ' ' . $fileEntry->user->lastname;
        $data['avatar'] = asset($fileEntry->user->avatar);
    } else {
        $data['name'] = lang('Anonymous', 'image page');
        $data['avatar'] = asset('images/avatars/default.png');
    }
    return responseHandler($data);
}

function direct_path($path)
{
...
function brokenFile()
{
    $path = 'images/sections/broken.png';
    $file = File::get($path);
