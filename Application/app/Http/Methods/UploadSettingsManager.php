...
class UploadSettingsManager
{
    ...
    private static function users()
    {
		...
		$paywall = User::where('id', Auth::user()->id)->value('storage');
		
        $storageSpace = $paywall;
    ...
    private static function storageFullness($uploadSetting)
    {
		...
		$paywall = $uploadSetting->storage_space;
		}
		
        $fullnessPercentage = $paywall ? (static::getClientUsedSpace() * 100) / $paywall : 0;
