...
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.settings.upload.update', $uploadSetting->id) }}" method="POST">
        @csrf
        <div class="card custom-card mb-4">
            ...
            <ul class="custom-list-group list-group list-group-flush">
                        ...
                        <div class="col col-lg-4">
                            <div class="custom-input-group input-group plan-storage-space">
                                <input type="number" name="storage_space" class="form-control" placeholder="0"
                                    value="{{ calculateBytes($uploadSetting->storage_space) }}" required
                        ...
                        <div class="col col-lg-4">
                            <div class="custom-input-group input-group plan-file-size">
                                <input type="number" name="file_size" class="form-control" placeholder="0"
                                    value="{{ calculateBytes($uploadSetting->file_size) }}" required
                                    {{ !$uploadSetting->file_size ? 'disabled' : '' }}>
                                <span class="input-group-text {{ !$uploadSetting->file_size ? 'disabled' : '' }}"><strong><i
                                            class="fas fa-hdd me-2"></i>{{ __('MB') }}</strong></span>
                            </div>
                        </div>
                    </div>
                ...
                <li class="list-group-item">
                    <div class="row align-items-center">
                        <div class="col-8 col-lg-8">
                            <label class="col-form-label d-block"><strong>{{ __('Allow downloading') }} :
                                </strong></label>
                            <small class="text-muted">{{ __('Allow downloading the Images') }}</small>
                        </div>
                        <div class="col-4 col-lg-4">
                            <input type="checkbox" name="allow_downloading" data-toggle="toggle"
                                data-on="{{ __('Yes') }}" data-off="{{ __('No') }}"
                                {{ $uploadSetting->allow_downloading ? 'checked' : '' }}>
                        </div>
                    </div>
                </li>
                <li class="list-group-item">
