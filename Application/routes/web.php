...
Route::get('ib/{filename}', 'Frontend\File\ViewFileController@secure')->name('file.secure');
Route::get('oembed', 'Frontend\File\Embed@show')->name('embed.show');
Route::group(localizeOptions(), function () {
    ...
    Route::group(['prefix' => 'user', 'namespace' => 'Frontend\User', 'middleware' => ['auth', 'verified', '2fa.verify']], function () {
        ...
        Route::name('user.')->group(function () {
            ...
            Route::post('subscription', 'PlanController@subscription')->name("subscription.create");
            Route::get('cancel', 'PlanController@cancel')->name("plan.cancel");
            Route::get('dashboard/charts/uploads', 'DashboardController@uploadsChart')->middleware('ajax.only');
            Route::get('dashboard/charts/views', 'DashboardController@viewsChart')->middleware('ajax.only');
            Route::name('gallery.')->prefix('gallery')->group(function () {
                Route::get('/', 'GalleryController@index')->name('index');
                Route::get('{shared_id}/edit', 'GalleryController@edit')->name('edit');
                Route::post('{shared_id}/update', 'GalleryController@update')->name('update');
                Route::get('{shared_id}/download', 'GalleryController@download')->name('download');
                Route::delete('{shared_id}/delete', 'GalleryController@destroy')->name('destroy');
                Route::post('delete/all', 'GalleryController@destroyAll')->name('destroy.all');
            });
    ...
    Route::group(['namespace' => 'Frontend', 'middleware' => ['verified', '2fa.verify']], function () {
        ...
        Route::name('file.')->namespace('File')->group(function () {
            Route::get('{shared_id}', 'ViewFileController@index')->name('view');
